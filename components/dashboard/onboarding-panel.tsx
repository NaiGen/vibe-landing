 ...
 *
 * `remaining` сверяется со ВСЕМ `PLACEHOLDER_LIST` (шесть записей),
 * а не с подмножеством, — но это всё равно **слабее** guard-теста, и знать
 * об этом надо: панель смотрит на шесть полей `SITE`, а guard сканирует
 * `lib/ app/ components/ scripts/ public/` целиком. Заглушка, оставшаяся
 * где-нибудь вне `lib/site.ts`, панель не удержит: она исчезнет, а
 * `pnpm test` останется красным. Так и задумано — панель про конфиг
 * и служит подсказкой, а судья готовности один, и это `pnpm test`.
 *
